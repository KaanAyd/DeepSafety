...
def validate(
    ...
    for data, target in val_loader:
        ...
        with torch.no_grad():
            ...
            correct += (predicted == target).sum().item()
             ...
             #newcommand
            true_negatives += ((predicted == target) & (predicted == 0)).sum().item()
            false_positives += ((predicted != target) & (predicted == 1)).sum().item()
            false_negatives += ((predicted != target) & (predicted == 0)).sum().item()
            true_positives += ((predicted == target) & (predicted == 1)).sum().item()
 
    validation_loss /= len(val_loader.dataset)
    lr_scheduler.step(round(validation_loss, 2))
    val_accuracy = correct / len(val_loader.dataset)
    val_specificity = true_negatives / (true_negatives + false_positives) if (true_negatives + false_positives) != 0 else 0
    validation_precision = true_positives / (true_positives + false_positives) if (true_positives + false_positives) != 0 else 0
    val_f1_score = 2*true_positives / (2*true_positives+false_positives+false_negatives) if (2*true_positives+false_positives+false_negatives) !=0 else 0
 
    #newcommand equation for validation_precision
    validation_precision = true_positives / (true_positives + false_positives) if (true_positives + false_positives) != 0 else 0
 
    print(
        f"Safety batch: Average loss: {validation_loss:.4f}, Accuracy: {100.0 * val_accuracy:.1f} %, Precision: {100.0 * validation_precision:.1f} %"
    )
    ...
    #newcommand
    mlflow.log_metric("validation precision", validation_precision, step=epoch)
    mlflow.log_metric("validation specificity", val_specificity, step=epoch)
    mlflow.log_metric("validation F1-score", val_f1_score, step=epoch)
